...
public interface MembershipRepository extends JpaRepository<Membership, UUID> {
    @Query(value = "SELECT mb FROM Membership mb JOIN FETCH mb.user JOIN FETCH mb.organization WHERE mb.organization.id = :organizationId AND mb.deletedAt IS NULL AND mb.user.deletedAt IS NULL",
    countQuery = "SELECT COUNT(mb) FROM Membership mb WHERE mb.organization.id = :organizationId AND mb.deletedAt IS NULL AND mb.user.deletedAt IS NULL")
    ...
    @Query("SELECT mb FROM Membership mb JOIN FETCH mb.user WHERE mb.id = :membershipId AND mb.organization.id = :organizationId AND mb.deletedAt IS NULL AND mb.user.deletedAt IS NULL")
